...
# 초기 추정값 설정
beta_guess = 10^(-7)  # beta 초기 추정값 설정
p_guess = 200     # p 초기 추정값 설정
p0 = [beta_guess, p_guess]

# curve_fit을 사용하여 파라미터 추정
params, params_covariance = curve_fit(removal_model, T_data, V_data, p0=p0)
...
# 시각화
t = np.linspace(0, 1, len(T_data))  # 임의의 시간 배열 생성
plt.figure()
plt.scatter(t, V_data, label='Data')
plt.plot(t, removal_model(t, estimated_beta, estimated_p), label='Fitted model', color='red')
plt.xlabel('Time')
